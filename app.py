...
def get_latest(d, sid):
    ...
    try:
        with d["S"]() as s:
            return s.query(d["M"]).filter(d["M"].strategy_id == sid).order_by(d["M"].updated_at.desc()).first()
    except: return None
...
def save_ok(d, rid, rec, sig, chart, details=None):
    ...
    try:
        with d["S"]() as s:
            r = s.get(d["M"], rid)
            if r:
                r.recommendation=rec; r.signal=sig; r.chart_path=chart
                r.details=details; r.status="success"; r.updated_at=datetime.utcnow()
                s.commit()
    except: pass
...
def trigger(strategy, db):
    ...
    def _run():
        ...
        odir = Path(__file__).parent / "outputs" / sid
        ...
        with col:
            with st.container(border=True):
                ...
                if lat and lat.status == "success":
                    ...
                    if lat.details:
                        with st.expander("詳細說明"): st.write(lat.details)
